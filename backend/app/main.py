...
async def lifespan(app: FastAPI):
    ...
    redis = await FastAPILimiter.init(
        f"redis://{settings.REDIS_HOST}:{settings.REDIS_PORT}", prefix="fastapi-limiter"
    )
    yield
...
def get_snapshots(
    ...
    return schemas.ArchiveResponse(
        domain=domain, snapshots=snapshot_responses, total=len(snapshot_responses)
    )
...
def get_snapshot(domain: str, timestamp: str, db: Session = Depends(get_db)):
    snapshot = (
        ...
        .filter(
            models.Snapshot.domain == domain, models.Snapshot.timestamp == timestamp
        )
        .first()
    )
...
def export_snapshots(
    ...
    if export_request.format == "json":
        ...
        return StreamingResponse(
            ...
            headers={
                "Content-Disposition": f"attachment; filename={domain}_snapshots.json"
            },
        )
        ...
        return StreamingResponse(
            ...
            media_type="text/csv",
            headers={
                "Content-Disposition": f"attachment; filename={domain}_snapshots.csv"
            },
        )
        ...
        return StreamingResponse(
            ...
            headers={
                "Content-Disposition": f"attachment; filename={domain}_snapshots.zip"
            },
        )
